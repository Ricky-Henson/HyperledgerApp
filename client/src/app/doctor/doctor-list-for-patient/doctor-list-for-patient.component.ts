...
export class DoctorListForPatientComponent implements OnInit, OnDestroy {
  ...
  public fetchDoctorData(): void {
    this.allSubs.add(
      this.doctorService.getDoctorsByOfficeId(1).subscribe((x) => {
        const data = x as Array<DoctorRecord>;
        data.map((y) => this.doctorRecords.push(new DoctorViewRecord(y)));
      })
    );
    this.allSubs.add(
      this.doctorService.getDoctorsByOfficeId(2).subscribe((x) => {
        const data = x as Array<DoctorRecord>;
        data.map((y) => this.doctorRecords.push(new DoctorViewRecord(y)));
      })
    );
    this.allSubs.add(
      this.doctorService.getDoctorsByOfficeId(3).subscribe((x) => {
        const data = x as Array<DoctorRecord>;
        data.map((y) => this.doctorRecords.push(new DoctorViewRecord(y)));
      })
    );
  ...
  public grant(doctorId: string): void {
    this.allSubs.add(
      this.patientService
        .grantAccessToDoctor(this.patientID, doctorId)
        .subscribe((x) => {
          console.log(x);
          this.refresh();
        })
  ...
  public revoke(doctorId: string): void {
    this.allSubs.add(
      this.patientService
        .revokeAccessFromDoctor(this.patientID, doctorId)
        .subscribe((x) => {
          console.log(x);
          this.refresh();
        })
