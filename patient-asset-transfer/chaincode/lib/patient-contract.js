...
class PatientContract extends PrimaryContract {
   ...
   * @param  {Context} ctx
   * @param  {JSON} args containing patientId and employeeId
   * @description Add the employee to the permissionGranted array
   */
  async grantAccessToEmployee(ctx, args) {
    args = JSON.parse(args);
    let patientId = args.patientId;
    ...
    const patient = await this.readPatient(ctx, patientId);
    // unique employeeIDs in permissionGranted
    if (!patient.permissionGranted.includes(employeeId)) {
      patient.permissionGranted.push(employeeId);
      patient.changedBy = patientId;
    }
    const buffer = Buffer.from(JSON.stringify(patient));
   ...
   * @param  {Context} ctx
   * @param  {JSON} args containing patientId and employeeId
   * @description Remove the employee from the permissionGranted array
   */
  async revokeAccessFromEmployee(ctx, args) {
    args = JSON.parse(args);
    let patientId = args.patientId;
    let employeeId = args.employeeId;
    ...
    // Get the patient asset from world state
    const patient = await this.readPatient(ctx, patientId);
    // Remove the employee if existing
    if (patient.permissionGranted.includes(employeeId)) {
